...
export function Countdown({ countdownDate }: { countdownDate: string }) {
  ...
  if (!timeLeft) {
    return (
      <p className="glass relative mt-10 rounded-2xl px-6 py-4 font-serif text-lg italic text-green md:text-xl">
        The day has arrived!
      </p>
    );
  }
    ...
    <div className="relative mt-10 grid w-full max-w-md grid-cols-4 gap-3 md:mt-12 md:max-w-lg md:gap-4">
      {units.map(({ key, label }) => (
        ...
        >
          ...
          <span className="mt-1 text-[10px] uppercase tracking-wider text-green/60 md:text-xs">
            {label}
          </span>
        </div>
      ))}
